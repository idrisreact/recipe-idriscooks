import { pgTable, text, integer, timestamp, boolean, decimal, jsonb, index, unique } from "drizzle-orm/pg-core";
import { relations } from "drizzle-orm";
import { user } from "./user.schema";

// Subscription Plans - Define different pricing tiers and their features
...
// User Subscriptions - Track current user subscription status  
...
// User Usage Tracking - Monitor usage against plan limits
...
// Subscription History - Audit trail for plan changes
...
export const subscriptionPlansRelations = relations(subscriptionPlans, ({ many }) => ({
  ...
  subscriptionHistoryFrom: many(subscriptionHistory, { relationName: "subscriptionHistoryFromPlan" }),
...
export const billingHistoryRelations = relations(billingHistory, ({ one }) => ({
  ...
  subscription: one(userSubscriptions, { fields: [billingHistory.subscriptionId], references: [userSubscriptions.id] }),
}));
